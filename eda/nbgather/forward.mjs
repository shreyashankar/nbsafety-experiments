...
db.all(sql, [], (err, rows) => {
    ...
    let dependencies = new Map(linenos.map((lineno) => {
        let locations = slice(ast, new LocationSet(loc(lineno, 1000)));
        // console.log(lineno);
    ...
    console.assert(cellToRerunStart <= cellToRerunEnd, "end is less than start");

    // assert here that cell is expected
    let slicedCell = splitSource.slice(cellToRerunStart, cellToRerunEnd + 1);
    console.assert(slicedCell == source[cellToRerun], "Cell is not expected.");

    // console.log(source[cellToRerun]);
    // console.log(splitSource[cellToRerunEnd]);
    // console.log(splitSource[cellToRerunStart]);

    // Iterate through values
    var forwardSliceLines = new Set();
    for (let [lineno, deps] of dependencies) {
        for (const dep of deps) {
            if (cellToRerunStart <= dep && dep <= cellToRerunEnd) {
    ...
    forwardSliceLines = Array.from(forwardSliceLines)
    forwardSliceLines.sort();

    // Create forward slice
    var forwardSlice = [];
    for (const line of forwardSliceLines) {
        forwardSlice.push(splitSource[line]);
    }
    console.log(forwardSlice);
