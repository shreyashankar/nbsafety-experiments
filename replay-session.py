...
ORDER BY counter ASC
    ...
    random_like_new_refresher_stats = ReplayStatsGroup(
        "random_like_new_refresher_cells"
    )
    ...
    if args.use_nbsafety:
        ...
        safety.config.backwards_cell_staleness_propagation = (
            not args.forward_only_propagation
        )
        logger.info(
            "backwards staleness propagation: %s"
            % safety.config.backwards_cell_staleness_propagation
        )
        safety.config.naive_refresher_computation = args.naive_refresher_computation
        logger.info(
            "naive refresher computation: %s"
            % safety.config.naive_refresher_computation
        )
...
{cell_source}
    ...
    logger.warning(traceback.format_exc())""".strip()
        try:
            cell_source = black.format_file_contents(
                cell_source, fast=False, mode=black.FileMode()
            )
        except:  # noqa
            pass
        ...
        try:
            ...
            start_time = timer()
            this_cell_had_safety_errors = timeout_run_cell(
                cell_id, cell_source, safety=safety
            )
        ...
        if (
            safety is not None
            and prev_cell_id is not None
            and cell_id != prev_cell_id
            and cell_id in notebook_state
        ):
            if should_test_prediction:  # and not this_cell_had_safety_errors:
                ...
                if True:  # cell_id != prev_cell_id + 1:
                    ...
                    refresher_stats.update(
                        cell_id, refresher_cells, num_available_cells
                    )
                    new_or_refresher_stats.update(
                        cell_id, refresher_cells | new_live_cells, num_available_cells
                    )
                    new_refresher_cells = refresher_cells - prev_refresher_cells
                    new_refresher_stats.update(
                        cell_id, new_refresher_cells, num_available_cells
                    )
                    random_like_new_refresher_stats.update(
                        cell_id, len(new_refresher_cells), notebook_state.keys()
                    )
                    stale_stats.update(cell_id, stale_cells, num_available_cells)
                    new_stale_stats.update(
                        cell_id, stale_cells - prev_stale_cells, num_available_cells
                    )
        ...
        if safety is not None:
            ...
            start_time = timer()
            precheck = safety.check_and_link_multiple_cells(
                notebook_state, order_index_by_cell_id=cell_order_idx
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--trace",
        type=int,
        help="Which trace the session to run is in",
        required=True,
    )
    parser.add_argument(
        "-s", "--session", type=int, help="Which session to run", required=True
    )
    parser.add_argument(
        "--use-nbsafety",
        "--nbsafety",
        action="store_true",
        help="Whether to use nbsafety",
    )
    parser.add_argument(
        "--log-to-stderr",
        "--stderr",
    ...
    parser.add_argument(
        "--just-log-files",
        action="store_true",
        help="If true, just log paths of files w/out running",
    )
    parser.add_argument(
        "--just-log-imports",
        action="store_true",
        help="If true, just log imports w/out running",
    )
    parser.add_argument(
        "--write-session-file",
        action="store_true",
        help="If true, write session to .py file",
    )
    parser.add_argument(
        "--write-session-ipynb",
        action="store_true",
        help="If true, write session to .ipynb file",
    )
    parser.add_argument(
        "--no-stats-logging",
        action="store_true",
        help="No writing to db tables if true",
    )
    parser.add_argument(
        "--forward-only-propagation",
        action="store_true",
        help="Only propagate staleness forwards if true",
    )
    parser.add_argument(
        "--naive-refresher-computation",
        action="store_true",
        help="Use quadratic refresher computation if true",
    )
